...
const FLEET = [
  {
    name: "Caonabo A320neo",
    kind: "Moyen-courrier",
    pax: "180 passagers",
    range: "6 300 km d'autonomie",
    desc: "L'épine dorsale de notre réseau régional : idéal pour les liaisons Santiago–Lima et les vols intérieurs des Caraïbes.",
  },
  {
    name: "Caonabo A330-900",
    kind: "Long-courrier",
    pax: "287 passagers",
    range: "13 300 km d'autonomie",
    desc: "Notre appareil des grandes traversées, qui relie sans détour Haïti au Canada et à l'Amérique du Sud.",
  },
  {
    name: "Caonabo A220-300",
    kind: "Régional",
    pax: "140 passagers",
    range: "6 300 km d'autonomie",
    desc: "Léger et sobre en carburant, parfait pour les dessertes plus fines vers Cap-Haïtien et les escales insulaires.",
  },
];

const CABINS = [
  { name: "Économique", color: "#5b21b6", desc: "Sièges confortables, en-cas et boissons offerts, franchise bagage cabine incluse." },
  { name: "Premium", color: "#7c4dd6", desc: "Plus d'espace pour les jambes, embarquement prioritaire, repas amélioré et bagage en soute inclus." },
  { name: "Business", color: "#dc2626", desc: "Sièges inclinables, service au champagne, restauration premium et double franchise bagages." },
];
    ...
    <div style={{ maxWidth: 1536, margin: "0 auto", background: "#fff" }}>
      ...
      <div className="hz" style={{ padding: "48px 56px 0" }}>
        ...
        <p style={{ color: "#5c5c7a", fontSize: 16, marginTop: 10, maxWidth: 680 }}>
          Des appareils modernes et sobres, choisis pour relier confortablement chaque
      ...
      <div className="hz" style={{ padding: "36px 56px 60px", display: "flex", flexDirection: "column", gap: 24 }}>
        {FLEET.map((a) => (
          <section key={a.name} style={pageCard}>
            <div className="fleet-row" style={{ display: "grid", gridTemplateColumns: "1fr 1.1fr", gap: 32, alignItems: "center" }}>
              ...
              <div>
                ...
                <h2 className="font-heading" style={{ ...pageH2, marginBottom: 8 }}>{a.name}</h2>
                <p style={{ ...pageP, marginBottom: 16 }}>{a.desc}</p>
                <div style={{ display: "flex", gap: 26, flexWrap: "wrap" }}>
                  <Spec icon="👥" label="Capacité" value={a.pax} />
                  <Spec icon="🌍" label="Rayon d'action" value={a.range} />
                </div>
              </div>
            </div>
          </section>
        ...
        <section style={pageCard}>
          ...
          <div className="grid-2 cabin-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 18, marginTop: 6 }}>
            {CABINS.map((c) => (
